...
module.exports = function resampleToPng(localInputPath, outputUri, options, callback) {
  try {
  ...
  return output(outputUri, callback, function(err, localOutputPath, done) {
    ...
    return tmp.tmpName({
      ...
      resampleOutputPath = path.join(path.dirname(localOutputPath), path.parse(localOutputPath).name + ".tif");
      if (err) {
        return callback(err);
      }
